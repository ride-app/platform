 ...
 */
export function updateDefaultComputeServiceAccount(projectNumber: string = gcp.config.project!) {
  ...
  return svcAccRoles.map(
    (role, idx) =>
      new gcp.projects.IAMMember(`default-compute-service-account-iam-member-${idx}`, {
        role: role,
